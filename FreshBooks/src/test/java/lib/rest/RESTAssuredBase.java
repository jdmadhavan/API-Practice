...
public class RESTAssuredBase extends PreAndTest{
   ...
   public static String getjsonKeyvalue(File file, String name) throws FileNotFoundException, ParseException
   {
	...
	public static RequestSpecification setgetsLogs() {
		return RestAssured
				.given()
				.log()
				.all();
	...
	public static RequestSpecification setGetLog() {
		return RestAssured
				.given()
				.log()
				.all();
	...
	public static Response putWithBodyAsStringAndUrl(File bodyFile, String URL) {
				return setLogs()
				.body(bodyFile)
				.put(URL);
	}
	...
	public static void verifyContentsWithKey(Response response, String key, int expVal) {
		boolean valexist=false;
		if(response.getContentType().contains("json")) {
			JsonPath jsonPath = response.jsonPath();
			List<Integer> actValue = jsonPath.getList(key);
			for (int i=0;i<actValue.size();i++)
			{
			if(actValue.get(i)==expVal) {
				valexist=true;
			}
			}
